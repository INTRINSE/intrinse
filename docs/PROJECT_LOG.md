...
- **Pre-launch Fix 8 — Resolve mobile scroll jank on ENERGY page** (`Pre-launch Fix 8 – Resolve mobile scroll jank on ENERGY page`) — Reproduzierter Fehler: auf `energy.html` ruckelten/sprangen "STATES", "ENERGY." und "Calm activation." beim Zurückscrollen zum Preis-/CTA-Bereich auf Mobile (getestet 390×844, 393×852, 430×932). Root Cause empirisch bestätigt (Playwright, Bounding-Rect- und `classList`-Protokollierung über mehrere Scroll-Zyklen): der `IntersectionObserver` in `energy.html` entfernte die Klasse `vis` jedes Mal, wenn ein `.sr`-Element den Viewport verließ — auch nach oben. Da `.frame{min-height:auto}` auf Mobile greift, ist der `#product`-Bereich höher als ein Viewport; bereits das Scrollen zum Preis-/CTA-Bereich schiebt den Kopfbereich (STATES/ENERGY/Calm activation.) aus dem sichtbaren Bereich, wodurch `vis` entfernt wurde. Beim Zurückscrollen wurde die 1s-Reveal-Transition (Opacity/TranslateY) dadurch jedes Mal neu abgespielt — das war das beobachtete Ruckeln/Springen. Kein Performance-/Compositing-Problem, keine Bild-/Font-Ladeursache, keine Sticky-/Fixed-Interferenz. Kleinstmöglicher Fix (reines JS, keine CSS-Änderung): im `IntersectionObserver`-Callback wird `vis` nur noch beim ersten Sichtbarwerden gesetzt und das Element danach per `io.unobserve()` nicht mehr weiter beobachtet — die initiale Eintrittsanimation bleibt unverändert, ein erneutes Entfernen/Wiederabspielen ist strukturell ausgeschlossen. Änderung ist auf `energy.html` beschränkt (eigene, unabhängige Kopie des Observer-Codes; `index.html` unberührt). Verifiziert: 5 Scroll-Zyklen (abwechselnd langsam/schnell) auf allen 3 Mobile-Viewports, Halten am CTA-Bereich, Reload, Tablet (820×1180), Desktop (1440×900) — überall stabil (`opacity:1`, keine Transform-Restauration, feste Position), keine Regression an Produktbild/Preis/Button/Navigation, kein horizontaler Overflow, Konsole fehlerfrei. Nachgemeldet: ein Rückscroll-Ruckeln speziell beim STATES-Pfeil wurde vom Gründer nach diesem Fix noch einmal beobachtet; ein realistischerer Playwright-Nachtest (inkrementelles Wheel-Scrollen statt Sprüngen, `layout-shift`-Observer, 260+ Frame-Samples, wiederholtes schnelles Kreuzen der Schwelle) konnte das ursprüngliche Muster nicht erneut reproduzieren (`.back` blieb durchgehend bei `opacity:1`/Identitäts-Transform). Ursache nicht abschließend geklärt — mögliche Erklärungen (noch nicht verifiziert): veralteter Stand auf der getesteten Umgebung, oder ein von der Reveal-Animation unabhängiger, iOS-Safari-spezifischer Effekt (z. B. `backdrop-filter`/Blur auf der fixierten Nav während des Scrollens), der sich in der hier verfügbaren Chromium-Testumgebung nicht nachstellen lässt. Bleibt offener Punkt, siehe Abschnitt 3.
- **Pre-launch Fix 9 — Small Business Price Notice** (`Pre-launch Fix 9 – Small Business Price Notice`) — Kleinunternehmer-Hinweis (§ 19 UStG) auf `energy.html` ergänzt, unter Verwendung der bereits vorhandenen, bis dahin ungenutzten Klasse `.p-vat` (keine CSS-Änderung nötig, Typografie/Farbe entsprachen bereits `.p-ship`: 11.5px, `var(--ink-soft)`, Gewicht 300). Formulierung vor Umsetzung vom Gründer freigegeben: *"Gemäß § 19 UStG wird keine Umsatzsteuer ausgewiesen."* Platzierung: direkt unter `.p-price`, vor dem Kaufbereich — bewusst nachgeordnet, ohne den Preis als visuellen Mittelpunkt zu verändern. Keine neue Farbe, kein Fettdruck, keine Icons, keine zusätzlichen Abstände. Verifiziert: Mobile/Tablet/Desktop, Schriftgröße/Farbe/Gewicht identisch mit `.p-ship` bestätigt, kein horizontaler Overflow, Konsole fehlerfrei.
- **Pre-launch Fix 10 — Restore Shopify Buy Button integration** (`Pre-launch Fix 10 – Restore Shopify Buy Button integration`) — Vollständige Untersuchung der Shopify-Buy-Button-Integration anhand der aktuellen offiziellen Shopify-Dokumentation (shopify.dev, js-buy-sdk/buy-button-js GitHub Pages), nicht anhand von Drittquellen. Drei unabhängige, empirisch belegte Root Causes gefunden (nicht vermutet): **(1) Skript-URL veraltet** — `cdn.shopify.com/shopifycloud/buy-button-storefront/dist/buy-button.min.js` liefert weiterhin 404; die offizielle, aktuell live bestätigte URL lautet `sdks.shopifycdn.com/buy-button/latest/buy-button-storefront.min.js` (per `curl` gegen die echte Shopify-CDN-Infrastruktur verifiziert, `200`, korrektes `ShopifyBuy`-Bundle). **(2) Falscher ID-Typ in `createComponent`** — der Code übergab `SHOPIFY.variants.ENERGY` (eine Varianten-ID) an `ui.createComponent('product', {id: …})`, das aber eine Produkt-ID erwartet; per Storefront-API-Live-Abfrage gegen den echten Shop (`zsuega-bp.myshopify.com`, hinter `intrinse.eu` bestätigt) verifiziert: `54898857640265` ist die ProductVariant-ID des Produkts "ENERGY", die tatsächliche Produkt-ID lautet `10310119686473`. Die Varianten-ID bleibt unverändert für den funktionierenden Cart-Permalink (`/cart/{variant_id}:1`) im Fallback in Verwendung. **(3) Storefront-Access-Token ungültig** — derselbe Token liefert sowohl über `intrinse.eu` als auch über die echte `.myshopify.com`-Domain identisch `401 UNAUTHORIZED`, exakt wie ein frei erfundener Test-Token; eine Anfrage ganz ohne Token-Header gelingt dagegen (liefert u. a. den echten Shop-Namen "INTRINSE" zurück) — das grenzt die Ursache eindeutig auf den Tokenwert selbst ein. **Dieser dritte Punkt konnte nicht behoben werden** — ein neuer, gültiger Storefront-Access-Token kann nur im Shopify-Adminbereich erzeugt werden, zu dem kein Zugriff besteht; bleibt offener Punkt (siehe Abschnitt 3). Zusätzlich beim Live-Test entdeckt und mitbehoben: die bestehende Fallback-Sicherung war zweifach fragil — (a) `document.getElementById('fallbackBtn').style.display='none'` wurde durch die bestehende CSS-Regel `.cart button{display:flex!important}` immer schon überschrieben (der Fallback-Button ließ sich in der Praxis nie wirklich per JS ausblenden, unabhängig vom Token) und (b) selbst mit korrigiertem `!important`-Override hätte das bloße Erscheinen des vom SDK sofort eingefügten (aber bei fehlgeschlagenem Request leeren) `<iframe>`-Gerüsts fälschlich als Erfolg gegolten und den Fallback ausgeblendet, obwohl das Widget nichts anzeigt — mit ungültigem Token wäre der Checkout dann vollständig blockiert gewesen (schlimmer als der jetzige Zustand, in dem der Fallback zufällig sichtbar bleibt, weil das Skript gar nicht erst lädt). Fix: Fallback wird jetzt erst per `style.setProperty('display','none','important')` ausgeblendet, nachdem ein Polling (alle 250 ms, max. 5 s) bestätigt hat, dass im Iframe selbst tatsächlich Inhalt gerendert wurde (`iframe.contentDocument.body.childElementCount > 0`) — leeres Iframe-Gerüst allein zählt nicht als Erfolg. Empirisch für beide Fälle verifiziert (echter fehlgeschlagener Request lässt Fallback dauerhaft sichtbar; simulierter Erfolg durch injizierten Iframe-Inhalt blendet ihn korrekt aus). Offizieller Embed-Vergleich (help.shopify.com) zeigt zusätzlich `apiKey`/`appId`-basierte Auth als Alternative zu `storefrontAccessToken` — das ist ein separater, gleichwertig unterstützter Auth-Weg für die "Buy Button"-Sales-Channel-App, nicht relevant für dieses Setup mit eigenem Storefront-Token; keine Umstellung vorgenommen. Geänderte Datei: ausschließlich `energy.html` (Script-URL, `SHOPIFY.products.ENERGY` ergänzt, `id` in `createComponent`, Fallback-Sichtbarkeitslogik). Verifiziert: Mobile/Tablet/Desktop, Reload, kein horizontaler Overflow, Preis/Varianten-ID/Fallback-Linkziel korrekt, keine visuelle Regression. Konsole zeigt weiterhin zwei Fehler (401 + eine daraus resultierende SDK-interne `TypeError`) — beide direkte Folge des ungültigen Tokens, nicht durch diesen Fix verursacht und ohne Admin-Zugriff nicht behebbar.
...
| Checkout-Livetest auf echter Domain | Noch nicht durchgeführt. Ursache des Shopify-Skript-Ladefehlers ist inzwischen **abschließend geklärt und die Skript-URL korrigiert** (Fix 10) — der eigentliche Livetest auf `intrinse.eu` steht weiterhin aus, zusätzlich blockiert durch den ungültigen Storefront-Token (siehe unten). | QA-Audit P0, Pre-launch Fix 6, Pre-launch Fix 10 |
| Shopify-Buy-Button-Skript-URL liefert 404 | **Behoben (Fix 10).** Korrekte, live verifizierte URL (`sdks.shopifycdn.com/buy-button/latest/buy-button-storefront.min.js`) eingesetzt. Zusätzlich einen zweiten, bis dahin unentdeckten Konfigurationsfehler gefunden und behoben: `createComponent` erhielt eine Varianten- statt Produkt-ID. Kein offener Punkt mehr bezüglich der URL/ID-Konfiguration. | Pre-launch Fix 10 |
| Storefront-Access-Token ungültig | **Neuer, bestätigter Launch-Blocker (Fix 10).** Der in `energy.html` hinterlegte Token liefert sowohl über `intrinse.eu` als auch über die echte `.myshopify.com`-Domain `401 UNAUTHORIZED` — identisch zu einem frei erfundenen Test-Token. Kann nur im Shopify-Adminbereich durch einen neuen, gültigen Storefront-Access-Token ersetzt werden (kein Zugriff von hier aus möglich). Bis dahin bleibt der Fallback-Button die einzige funktionierende Kaufmöglichkeit — dessen Zuverlässigkeit wurde in Fix 10 zusätzlich abgesichert. | Pre-launch Fix 10 |
...
| MwSt.-Hinweis beim Preis (`.p-vat` aktivieren) | **Umgesetzt (Fix 9).** Kein offener Punkt mehr. | Pre-launch Fix 4 + 9 |
...
| Scroll-Ruckeln beim STATES-Pfeil auf `energy.html` (Mobile) | Vom Gründer nach Fix 8 erneut beobachtet, in einem realistischeren automatisierten Nachtest (inkrementelles Scrollen statt Sprüngen) aber nicht reproduzierbar. Ursache nicht abschließend geklärt — möglich: veralteter Teststand oder ein von der ursprünglichen Reveal-Animation unabhängiger, iOS-Safari-spezifischer Effekt, der sich in der lokalen Chromium-Testumgebung nicht abbilden lässt. Weitere Diagnose (Testumgebung, Gerät/Browser, genaues Symptom) erforderlich, bevor ein weiterer Fix versucht wird. | Pre-launch Fix 8 (Nachtrag) |
